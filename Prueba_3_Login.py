...
class LoginScreen(Screen):
    ...
    def go_to_welcome(self, username):
        welcome_screen = None
        for screen in self.manager.screens:
        ...
        if welcome_screen is None:
            welcome_screen = WelcomeScreen(username=username)
            self.manager.add_widget(welcome_screen)

        welcome_screen.username = username
        self.manager.current = welcome_screen.name
...
class WelcomeScreen(Screen):
    def __init__(self, username, **kwargs):
        ...
        self.name = 'welcome'
        layout = BoxLayout(orientation='vertical', spacing=10, padding=10)
        layout.add_widget(Label(text=f"Bienvenido, {username}!"))
